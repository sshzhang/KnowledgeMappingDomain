...
public class DianpingCarNeo4jUtils extends Neo4jUtils<CateringCommentDomain, DianpingCateringApplicationDomain> {

    public DianpingCarNeo4jUtils(CateringCommentDomain CommenT, DianpingCateringApplicationDomain allStaticContent, int status) {
    ...
    protected boolean CreateApplicationCommentDataToNeo4jNode(final CateringCommentDomain CommentT) throws InterruptedException {
        ...
        try {
            session.writeTransaction(new TransactionWork<Object>() {
                ...
                public Object execute(Transaction transaction) {
                    try {
                        ...
                        transaction.run("match(ss:Car{shop_name:\""+shop_name+"\",shop_url:\""+shop_url+"\"}),(sc:Comments{shop_name:\""+shop_name+"\",shop_url:\""+shop_url+"\",comment_user_name:\""+CommentT.getComment_user_name()+"\",comment_time:\""+CommentT.getComment_time()+"\"}) merge(ss)-[:IncludeComments{name:\"爱车类别包含的评论\"}]->(sc)");
                        ...
                        if (comment_rate_tag !=null&&(!"".equals(comment_rate_tag))) {
                            ...
                            for (String str : strings) {
                                ...
                                transaction.run("match(sc:Comments{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\",comment_user_name:\""+CommentT.getComment_user_name()+"\",comment_time:\""+CommentT.getComment_time()+"\"}),(si:Shope_gust_impression{name:\"" + str + "\",shuxing:\"" + cateringCommentsRateTag.get(str) + "\"}) merge (sc)-[:CommentIncludeImpression{name:\"用户对爱车类别的评分\"}]->(si)");
                            }
                        }
            ...
            ex.printStackTrace();
            LogsUtils.WriteTheDataToFile(ex.getMessage(), "src/resources/carNeo4jContent.txt");
        }finally {
            try {
                session.close();
            } catch (Exception e) {
        ...
        try {
            ...
            session.writeTransaction(new TransactionWork<Object>() {
                ...
                public Object execute(Transaction tx) {
                    ...
                    try {
                        StatementResult run = tx.run("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) return labels(sf) as label,sf.crawl_time as crawl_time");
                        if (run.hasNext()) {//存在数据记录
                            ...
                            if (labels.contains(subtype_name)) {//存在相应的标签记录  看是否更新数据
                                ...
                                if (!ncrawl_time.equals(crawl_time)) { //时间不相等就更新
                                    System.out.println("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) set sf.shop_phone=\"" + shop_phone + "\",sf.crawl_time=\"" + crawl_time + "\",sf.shop_price=" + shop_price + ",sf.shop_address=\"" + shop_address + "\",sf.shop_time=\"" + shop_time + "\",sf.shop_comment_num=" + cateringDomain.getShop_comment_num() + "");
                                    tx.run("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) set sf.shop_phone=\"" + shop_phone + "\",sf.crawl_time=\"" + crawl_time + "\",sf.shop_price=" + shop_price + ",sf.shop_address=\"" + shop_address + "\",sf.shop_time=\"" + shop_time + "\",sf.shop_comment_num=" + cateringDomain.getShop_comment_num() + "");
                                    //先所有关系节点，再重新构建
                                    tx.run("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"})-[r]->(n) delete r,n");
    ...
    private void CreateCateringRelationship(Transaction tx, String shop_name, String shop_url, String id, Map<String, Float> shop_tags, DianpingshopMenuDomain dianpingshopMenuDomain, Map<String, DianpingCateringShopPromotionDomain> shopPromotionDomain, DianpingCateringShopStatisticDomain shopStatisticDomain) {
        if (shop_tags != null) {
            ...
            for (String key : keys) {
                ...
                System.out.println("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(si:Shope_gust_impression{id:\"" + id + "\",name:\"" + key + "\",shuxing:" + value + "}) merge(sf)-[:ShoppingIncludeShopTags{name:\"爱车类别包含的评价标签\"}]->(si) return si");
                tx.run("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(si:Shope_gust_impression{id:\"" + id + "\",name:\"" + key + "\",shuxing:" + value + "}) merge(sf)-[:ShoppingIncludeShopTags{name:\"爱车类别包含的评价标签\"}]->(si) return si");
            }
        }
        ...
        if (dianpingshopMenuDomain != null) {
            ...
            if (recommenDishList != null) {
                ...
                for (String key : keySet) {
                    ...
                    tx.run("create(sm:CarMenu{name:\"" + key + "\",picture:\"" + picture + "\",price:\"" + price + "\",number:\"" + suggestNumber + "\",id:\"" + id + "\"})");
                    tx.run("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(sm:CarMenu{name:\"" + key + "\",picture:\"" + picture + "\",price:\"" + price + "\",number:\"" + suggestNumber + "\",id:\"" + id + "\"}) merge(sf)-[:HealthIncludeMenu{name:\"爱车类别包含的菜单\"}]->(sm)");
                }
            }
            //TODO 环境  价目表
        }
        ...
        if (shopPromotionDomain != null) {
            ...
            for (String key : keySet) {
                ...
                tx.run("create(ss:Promotion{name:\"" + key + "\",currPrice:\"" + currPrice + "\",originalPrice:\"" + originalPrice + "\",hadSaled:\"" + hadSaled + "\",id:\"" + id + "\",type:\"爱车优惠促销\"})");
                tx.run("match(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(ss:Promotion{name:\"" + key + "\",currPrice:\"" + currPrice + "\",originalPrice:\"" + originalPrice + "\",hadSaled:\"" + hadSaled + "\",id:\"" + id + "\",type:\"爱车优惠促销\"}) merge (sf)-[:CarIncludePromotion{name:\"爱车类别包含的促销优惠\"}]->(ss)");
            }
        }
        if (shopStatisticDomain != null) {
            ...
            if (peopleAllThink != null && peopleAllThink.size() > 0) {
                ...
                for (DianpingCateringPeopleReviewDataDomain allstr : allStrs) {
                    ...
                    tx.run("match(si:Shope_gust_impression{name:\"" + name + "\",shuxing:" + attribute + ",id:\"" + id + "\"}),(sf:Car{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) merge (sf)-[:CarIncludeReviewNum{name:\"爱车类别包含的评论个数\"}]->(si)");
                }
            }
