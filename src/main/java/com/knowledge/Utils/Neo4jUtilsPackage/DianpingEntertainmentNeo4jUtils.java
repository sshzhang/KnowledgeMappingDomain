...
public class DianpingEntertainmentNeo4jUtils extends Neo4jUtils<CateringCommentDomain, DianpingCateringApplicationDomain> {

    public DianpingEntertainmentNeo4jUtils(CateringCommentDomain CommenT, DianpingCateringApplicationDomain allStaticContent, int status) {
            ...
            /*session.writeTransaction(new TransactionWork<Object>() {
                ...
                public Object execute(Transaction transaction) {
                    try {
                        ...
                        transaction.run("match(ss:Entertainment{shop_name:\""+shop_name+"\",shop_url:\""+shop_url+"\"}),(sc:Comments{shop_name:\""+shop_name+"\",shop_url:\""+shop_url+"\",comment_user_name:\""+CommentT.getComment_user_name()+"\",comment_time:\""+CommentT.getComment_time()+"\"}) merge(ss)-[:IncludeComments{name:\"娱乐包含的评论\"}]->(sc)");
                        ...
                        if (comment_rate_tag !=null&&(!"".equals(comment_rate_tag))) {
                            ...
                            for (String str : strings) {
                                ...
                                transaction.run("match(sc:Comments{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\",comment_user_name:\""+CommentT.getComment_user_name()+"\",comment_time:\""+CommentT.getComment_time()+"\"}),(si:Shope_gust_impression{name:\"" + str + "\",shuxing:\"" + cateringCommentsRateTag.get(str) + "\"}) merge (sc)-[:CommentIncludeImpression{name:\"用户对娱乐的评分\"}]->(si)");
                            }
                        }
                    } catch (Exception ex) {
                        transaction.failure();
                        ex.printStackTrace();
                        LogsUtils.WriteTheDataToFile(ex.getMessage(), "src/resources/entertainmentNeo4jContent.txt");
                    }
                    return null;
                }
            });*/
    ...
    protected boolean CreateApplicationStaticContentDataToNeo4jNode(DianpingCateringApplicationDomain allStaticContent) {
        ...
        return CreateDianpingEntertainmentDataToNeo4jNode(allStaticContent.getCateringDomain(), allStaticContent.getDianpingshopMenuDomain(), allStaticContent.getShopPromotionDomain(), allStaticContent.getShopStatisticDomain(),allStaticContent.getShop_tags());
    }

    public boolean CreateDianpingEntertainmentDataToNeo4jNode(final CateringDomain cateringDomain, final DianpingshopMenuDomain dianpingshopMenuDomain, final Map<String, DianpingCateringShopPromotionDomain> shopPromotionDomain, final DianpingCateringShopStatisticDomain shopStatisticDomain, final Map<String,Float> shop_tags) {
        ...
        try {
            ...
            transaction = session.beginTransaction();
            StatementResult run = transaction.run("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) return labels(sf) as label,sf.crawl_time as crawl_time");
            if (run.hasNext()) {//存在数据记录
                ...
                if (labels.contains(subtype_name)) {//存在相应的标签记录  看是否更新数据
                    ...
                    if (!ncrawl_time.equals(crawl_time)) { //时间不相等就更新
                        System.out.println("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) set sf.shop_phone=\"" + shop_phone + "\",sf.crawl_time=\"" + crawl_time + "\",sf.shop_price=" + shop_price + ",sf.shop_address=\"" + shop_address + "\",sf.shop_time=\"" + shop_time + "\",sf.shop_comment_num=" + cateringDomain.getShop_comment_num() + "");
                        transaction.run("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) set sf.shop_phone=\"" + shop_phone + "\",sf.crawl_time=\"" + crawl_time + "\",sf.shop_price=" + shop_price + ",sf.shop_address=\"" + shop_address + "\",sf.shop_time=\"" + shop_time + "\",sf.shop_comment_num=" + cateringDomain.getShop_comment_num() + "");
                        //先所有关系节点，再重新构建
                        transaction.run("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"})-[r]->(n) delete r,n");
                        CreateCateringRelationship(transaction, shop_name, shop_url, id, shop_tags, dianpingshopMenuDomain, shopPromotionDomain, shopStatisticDomain);
                    }
                } else {//不存在相应的记录   添加标签类别信息
                    transaction.run("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) set sf:" + subtype_name + " return sf");
                }
            } else {//不存在数据记录  添加
                transaction.run("create(sf:Entertainment:" + subtype_name + "{id:\"" + cateringDomain.get_id() + "\",shop_rate:\"" + shop_rate + "\",shop_name:\"" + shop_name + "\",crawl_time:\"" + ncrawl_time + "\",data_website:\"" + cateringDomain.getData_website() + "\",shop_price:" + shop_price + ",data_source:\"" + cateringDomain.getData_source() + "\",data_region:\"" + cateringDomain.getData_region() + "\",shop_url:\"" + shop_url + "\",shop_address:\"" + shop_address + "\",shop_time:\"" + shop_time + "\",shop_phone:\"" + shop_phone + "\"}) return sf");
                System.out.println("create(sf:Entertainment:" + subtype_name + "{id:\"" + cateringDomain.get_id() + "\",shop_rate:\"" + shop_rate + "\",shop_name:\"" + shop_name + "\",crawl_time:\"" + ncrawl_time + "\",data_website:\"" + cateringDomain.getData_website() + "\",shop_price:" + shop_price + ",data_source:\"" + cateringDomain.getData_source() + "\",data_region:\"" + cateringDomain.getData_region() + "\",shop_url:\"" + shop_url + "\",shop_address:\"" + shop_address + "\",shop_time:\"" + shop_time + "\",shop_phone:\"" + shop_phone + "\"}) return sf");
                CreateCateringRelationship(transaction, shop_name, shop_url, id, shop_tags, dianpingshopMenuDomain, shopPromotionDomain, shopStatisticDomain);
            }
            transaction.success();
        } catch (Exception ex) {
    ...
    private void CreateCateringRelationship(Transaction tx, String shop_name, String shop_url, String id, Map<String, Float> shop_tags, DianpingshopMenuDomain dianpingshopMenuDomain, Map<String, DianpingCateringShopPromotionDomain> shopPromotionDomain, DianpingCateringShopStatisticDomain shopStatisticDomain) {
        if (shop_tags != null) {
            ...
            for (String key : keys) {
                ...
                System.out.println("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(si:Shope_gust_impression{id:\"" + id + "\",name:\"" + key + "\",shuxing:" + value + "}) merge(sf)-[:EntertainmentIncludeShopTags{name:\"娱乐包含的评价标签\"}]->(si) return si");
                tx.run("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(si:Shope_gust_impression{id:\"" + id + "\",name:\"" + key + "\",shuxing:" + value + "}) merge(sf)-[:CateringIncludeShopTags{name:\"娱乐包含的评价标签\"}]->(si) return si");
            }
        }
        ...
        if (dianpingshopMenuDomain != null) {
            ...
            if (recommenDishList != null) {
                ...
                for (String key : keySet) {
                    ...
                    tx.run("create(sm:EntertainmentEnvironment{name:\"" + key + "\",picture:\"" + picture + "\",price:\"" + price + "\",number:\"" + suggestNumber + "\",id:\"" + id + "\"})");
                    tx.run("match(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}),(sm:EntertainmentEnvironment{name:\"" + key + "\",picture:\"" + picture + "\",price:\"" + price + "\",number:\"" + suggestNumber + "\",id:\"" + id + "\"}) merge(sf)-[:EntertainmentIncludeEnvironment{name:\"娱乐环境情况\"}]->(sm)");
                }
            }
            //TODO 环境  价目表
        }
        ...
        if (shopStatisticDomain != null) {
            ...
            if (peopleAllThink != null && peopleAllThink.size() > 0) {
                ...
                for (DianpingCateringPeopleReviewDataDomain allstr : allStrs) {
                    ...
                    tx.run("match(si:Shope_gust_impression{name:\"" + name + "\",shuxing:" + attribute + ",id:\"" + id + "\"}),(sf:Entertainment{shop_name:\"" + shop_name + "\",shop_url:\"" + shop_url + "\"}) merge (sf)-[:EntertainmentIncludeReviewNum{name:\"娱乐包含的评论个数\"}]->(si)");
                }
            }
